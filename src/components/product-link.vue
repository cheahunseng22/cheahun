...
const fetchArtists = async () => {
    ...
    try {
        ...
        artists.value = data;
        console.log('Total artists loaded:', data.length); // Add this line
    } catch (error) {
...
const handleArtistClick = (artistId) => {
    ...
    if (selectedArtist.value === artistId) {
        clearSelections();
    } else {
        fetchTracksByArtist(artistId);
    }
...
const selectArtistFromModal = (artistId) => {
    showArtistModal.value = false;
    fetchTracksByArtist(artistId);
};
...
<div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 shadow-sm">
    ...
    <div v-else>
        ...
        <!-- View All Button for Artists -->
        <div v-if="artists.length > 5" class="flex justify-center mt-4">
            <button
    ...
    <!-- Artist Tracks with Avatar Header (shows only 5 tracks, with See More button) -->
    <div v-if="selectedArtist && artistTracks.length > 0" class="mt-4 p-3 bg-white rounded-lg">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            <div 
                v-for="track in artistTracks.slice(0, 5)" 
                :key="track.id"
                @click="goToTrack(track.id)"
        ...
        <!-- See More Button for Tracks (when more than 5 tracks) -->
        <div v-if="artistTracks.length > 5" class="flex justify-center mt-4">
            <button
                @click="showAllTracksModal = true"
                class="group px-4 py-2 bg-white rounded-full border border-gray-300 text-gray-600 font-medium hover:bg-purple-50 transition-all duration-300 text-sm"
            >
                <span class="flex text-[11px] items-center gap-1">
                    See More ({{ artistTracks.length - 5 }} more tracks)
                    <svg class="w-3 h-3 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </span>
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)] custom-scrollbar">
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    <div 
                        ...
                        :key="artist.id"
                        @click="selectArtistFromModal(artist.id)"
        ...
        <div class="relative w-full max-w-4xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden animate-modalSlideUp">
            <div class="sticky top-0 bg-gradient-to-r from-purple-600 to-pink-600 p-4 flex justify-between items-center z-20">
                <div>
                    ...
                    <p class="text-white/80 text-sm">{{ selectedArtistData?.name }} - {{ artistTracks.length }} tracks available</p>
                </div>
                <button 
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)] custom-scrollbar">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    <div 
                        ...
                        :key="track.id"
                        @click="goToTrack(track.id); showAllTracksModal = false"  
                        class="bg-white rounded-lg p-3 cursor-pointer border border-gray-200 hover:border-purple-500 hover:shadow-lg transition-all hover:scale-105 flex items-center gap-3"
                    >
                        <div class="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-400 rounded-lg flex items-center justify-center text-white">
                            🎵
                        </div>
                        <div class="flex-1">
