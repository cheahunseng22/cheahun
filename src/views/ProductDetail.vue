...
<template>
  <div class="wrap bg-gradient-to-br from-blue-50 via-white to-indigo-50 min-h-screen w-full">
    <!-- Loading State -->
    ...
    <div v-else-if="error" class="flex justify-center items-center h-screen px-4">
      <div class="text-center max-w-md mx-auto p-8 bg-white rounded-2xl shadow-xl border border-red-100">
        <div class="text-7xl mb-4">😢</div>
        <p class="text-xl text-red-600 font-semibold mb-2">Track Not Found</p>
        <p class="text-gray-600 mb-4">{{ error }}</p>
        <p class="text-sm text-gray-500 mb-6">The track you're looking for doesn't exist or has been removed.</p>
        <button 
          @click="goBackToHome"
    ...
    <div v-else class="max-w-7xl mx-auto px-4 py-6 md:py-10">
        ...
        <div class="space-y-6">
          ...
          <div v-else class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            <div class="bg-gradient-to-r from-gray-600 to-gray-700 px-5 py-3">
              <h3 class="font-semibold text-white flex items-center gap-2">
                <span class="text-xl"><i class="fa-solid fa-compact-disc"></i></span> Music Video
              </h3>
            </div>
            <div class="p-8 text-center">
              <div class="text-5xl mb-3"><i class="fa-solid fa-compact-disc"></i></div>
              <p class="text-gray-500">No music video available for this track</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            ...
            <div class="flex items-center gap-3 mb-4">
              ...
              <p class="text-xl text-gray-600 font-medium">{{ product.artist_name }}</p>
            </div>
          ...
          <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            <h3 class="font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span class="text-xl"><i class="fa-solid fa-notes-medical"></i></span> Description
            </h3>
          ...
          <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            <h3 class="font-bold text-gray-800 mb-4 flex items-center gap-2">
              <span class="text-xl"><i class="fa-solid fa-circle-info"></i></span> Production Details
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div v-if="product.producer" class="bg-gray-50 rounded-xl p-3">
                <p class="text-xs text-gray-500 mb-1">Producer</p>
                <p class="font-semibold text-gray-800">{{ product.producer }}</p>
              </div>
              ...
              <div v-if="product.key_signature" class="bg-gray-50 rounded-xl p-3">
                <p class="text-xs text-gray-500 mb-1">Key</p>
                <p class="font-semibold text-gray-800">{{ product.key_signature }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            <h3 class="font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span class="text-xl"><i class="fa-solid fa-layer-group"></i></span> Category
            </h3>
            <div class="flex flex-wrap gap-2">
              <span class="bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 py-2 rounded-full text-sm font-medium">
                {{ product.category }}
              </span>
            </div>
          </div>
      ...
      <div class="mt-16">
        <div class="flex items-center justify-between mb-6">
          ...
          <RouterLink to="/" class="text-blue-600 hover:text-blue-700 text-sm font-medium flex items-center gap-1">
            View All
        ...
        <div v-if="relatedTracks.length === 0" class="text-center py-12 bg-white rounded-2xl shadow-md">
          <div class="text-5xl mb-3"><i class="fa-solid fa-compact-disc"></i></div>
          <p class="text-gray-500">No related tracks found</p>
        </div>
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          <productCart v-for="item in relatedTracks" :key="item.id" :product="item" />
        </div>
      </div>
